/**
 * Post-Migration: Seed Foundational Skills
 *
 * This script runs after the migration.sql completes.
 * It seeds the 11 foundational skills for customer account intelligence.
 * Uses upsert pattern - idempotent and safe to re-run.
 *
 * IMPORTANT: This imports the full skill data from foundational-skills.ts
 * which contains complete scopeDefinition with covers, futureAdditions, and notIncluded.
 */
...
// Full foundational skills data with complete scope definitions
// This is a copy of the data from prisma/seeds/foundational-skills.ts
// to avoid import issues in the migration context
...
async function main() {
  try {
    // Get default team (created by seed-minimal.js)
    const team = await prisma.team.findFirst({
      where: { slug: 'default' },
    });

    if (!team) {
      console.log('⚠ Default team not found. Skipping foundational skills seeding.');
      console.log('Run seed-minimal.js first to create default team.');
      return;
    }

    // Get default user or create one for audit trail
    let user = await prisma.user.findFirst({
      orderBy: { createdAt: 'asc' },
    });

    if (!user) {
      console.log('⚠ No users found. Creating system user for foundational skills audit trail.');
      user = await prisma.user.create({
        data: {
          email: 'system@transparent-trust.local',
          name: 'System',
          teamMemberships: {
            create: {
              teamId: team.id,
    ...
    console.log('\n📚 Seeding foundational skills for customer library...');
    ...
    for (const skillData of FOUNDATIONAL_SKILLS) {
      const attributes = {
        ...
        isFoundational: true,
        scopeDefinition: skillData.scopeDefinition, // Full scope definition
        auditLog: {
          entries: [
            {
              action: 'created',
      ...
      let skill;
      if (existing) {
        // Skip existing skills - don't overwrite user edits
        skipped++;
        console.log(`  ⏭ Skipped (already exists): ${skillData.title}`);
        continue;
      } else {
        // Create new skill
        skill = await prisma.buildingBlock.create({
          data: {
    ...
    console.log(
      `\n✓ Foundational skills seeding complete: ${created} created, ${skipped} skipped\n`,
    );
  } catch (error) {
    console.error('❌ Foundational skills seeding failed:', error);
    throw error;
  } finally {
