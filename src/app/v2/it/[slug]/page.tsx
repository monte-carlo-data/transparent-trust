...
interface ITSkillDetailPageProps {
  params: Promise<{ slug: string }>;
}

export default async function ITSkillDetailPage({ params }: ITSkillDetailPageProps) {
  const { slug } = await params;

  const skill = await getSkillWithRelations(slug, 'it');
  if (!skill) {
    notFound();
  }
  ...
  const [rawSourceAssignments, rawPendingAssignments, relatedSkills] = await Promise.all([
    ...
    getRelatedSkillsBySlug(skill.id, attrs?.relatedSlugs || [], 'it'),
  ]);
