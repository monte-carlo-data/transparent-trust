 ...
 * GTM Library Layout
 *
 * Shared layout for all GTM library tabs.
...
export default async function GTMLibraryLayout({ children }: LayoutProps) {
  const session = await getServerSession(authOptions);

  // Check library access
  if (!session?.user?.id || !(await canAccessLibrary(session.user.id, 'gtm'))) {
    redirect('/v2');
  }

  // Check if user is admin for this library
  const userIsAdmin = await isLibraryAdmin(session.user.id, 'gtm');
