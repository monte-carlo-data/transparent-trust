/**
 * GET /api/v2/integrations/zendesk/stage
 * POST /api/v2/integrations/zendesk/stage
 *
 * List and stage Zendesk tickets as sources.
 ...
 *
 * POST Body:
 *   - items: Array of tickets to stage
...
export async function GET(req: NextRequest) {
  try {
    ...
    const paramsResult = parseStageListParams(searchParams, 'zendesk');
    ...
    // Get staged sources
    const result = await zendeskHandler.getStagedSources(params);

    return NextResponse.json(result);
  } catch (error) {
    return logAndReturnError('Zendesk stage list', error, 'Zendesk');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Stage the items
    const result = await zendeskHandler.stageItems(items, libraryId, customerId);

    return NextResponse.json(result);
  } catch (error) {
    return logAndReturnError('Zendesk stage create', error);
  }
}
